...
import { ArrowLeft, Monitor, Loader, Download, AlertCircle, RefreshCw, Wifi, WifiOff } from 'lucide-react';
...
const STATUS: Record<string, { label: string; color: string; dot: string }> = {
  ...
  executing: { label: 'Capturing…',           color: 'text-indigo-400', dot: 'bg-indigo-400 animate-pulse' },
...
export default function AdminScreenshotPage() {
  ...
  async function capture() {
    setBusy(true);
    try { await adminApi.issueCommand(deviceId, 'take_screenshot'); await fetchCmds(); }
    ...
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-8 pt-8 pb-5 border-b border-gray-800">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-white">Screenshot</h1>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-xs text-gray-500">
          Uses the Accessibility Service to silently capture the current screen (Android 11+). No dialog shown to the user.
        </div>

        {loading ? <PageLoader theme="dark" /> : commands.length === 0 ? (
          <div className="text-center py-16 text-gray-600">
            <Monitor size={40} className="mx-auto mb-3 opacity-20" />
            <p>No screenshots yet. Click above to capture.</p>
          </div>
        ) : (
